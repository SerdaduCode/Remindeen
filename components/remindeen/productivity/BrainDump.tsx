    ...
    <div className="flex h-full flex-col gap-2.5 p-4">
      ...
      <form onSubmit={handleSubmit} className="flex gap-2">
        ...
        <button
          ...
          className="flex h-9 w-9 shrink-0 cursor-pointer items-center justify-center rounded-full border border-white/10 bg-white/5 text-white outline-none transition hover:bg-white/15 active:scale-90 focus-visible:ring-2 focus-visible:ring-white/40"
        >
      ...
      {!loading && !error && notes.length === 0 && (
        <p className="py-4 text-center text-xs text-white/40">{t("brainDump.empty_state")}</p>
      )}
        ...
        <div className="flex max-h-[130px] flex-col gap-1.5 overflow-y-auto glass-scrollbar">
          {notes.map((note) => {
            ...
            return (
              <div
                key={note.id}
                className="group flex items-start gap-2.5 rounded-lg bg-white/[0.06] px-3 py-2 ring-1 ring-white/10"
              >
                <span className={`mt-1.5 h-1.5 w-1.5 shrink-0 rounded-full ${theme.dot}`} />
                <span className="flex-1 text-xs text-white/85">{note.body}</span>
                <button
                  type="button"
